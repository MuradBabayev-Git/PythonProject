...
def index(request):
    ...
    context = {
        "page_title": "Home",
        "products": products_qs[:12],  # ← Ограничение 12 продуктов
        "categories": categories,      # ← Без ограничения (хорошо)
        "discount_products": products_qs.filter(discount_value__gt=0),  # ← Без ограничения
        "last_products": products_qs.order_by("-id")[:5],  # ← Ограничение 5 продуктов
    }
...
def about_view(request):
    try:
        about_content = AboutPage.objects.get(is_active=True)
    except AboutPage.DoesNotExist:
        # Создаем временный объект с default значениями
        about_content = AboutPage(
            experience="30 Years Of Experience",
            title="ABOUT US",
            subtitle="World Largest Online Shopping Marketplace For You.",
